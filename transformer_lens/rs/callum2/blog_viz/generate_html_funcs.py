...
def _get_color(importances):
    ...
    """
    # Handle case where importances is a single value
    single_elem = False
    if isinstance(importances, float) or (isinstance(importances, Tensor) and importances.numel() == 1):
        importances = [importances]
        single_elem = True

    importances_filtered = [i if (i is not None) else 0.5 for i in importances]
    bg_colors = sample_colorscale("RdBu", importances_filtered, low=0.0, high=1.0, colortype='tuple')
    bg_colors = ['#%02x%02x%02x' % tuple([int(255 * x) for x in bg_color]) for bg_color in bg_colors]
    text_colors = list(map(lambda i: 0 if abs(i - 0.5) <= 0.31 else 1, importances_filtered)) # 0 for black (default), 1 for white, 2 for grey
    for i, imp in enumerate(importances):
        if imp is None:
            bg_colors[i] = "white"
            text_colors[i] = 2
    return (bg_colors, text_colors) if not(single_elem) else (bg_colors[0], text_colors[0])
