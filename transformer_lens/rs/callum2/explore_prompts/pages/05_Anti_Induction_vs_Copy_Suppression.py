...
def plot_all_results(
    ...
    for model_name in MODEL_NAMES:
        ...
        for layer in range(model_scores.size(1)):
            for head in range(model_scores.size(2)):
                x = -model_scores[0, layer, head].item()
                y = -model_scores[1, layer, head].item()
                results_copy_suppression_ioi.append(x)
                results_anti_induction.append(y)
                ...
                # opacity.append(0.04 if ((x >= 0) and (y >= 0)) else 0.005)
    ...
    fig.update_traces(textposition='top center') # marker_opacity=opacity
...
cols = st.columns(2)
with cols[0]:
    pospos = st.checkbox("Only show pos-pos quadrant")
