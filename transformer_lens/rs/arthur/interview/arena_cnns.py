...
# 
# [Streamlit page](https://arena-ch0-fundamentals.streamlit.app/[0.2]_CNNs)
# 
# Please send any problems / bugs on the `#errata` channel in the [Slack group](https://join.slack.com/t/arena-la82367/shared_invite/zt-1uvoagohe-JUv9xB7Vr143pdx1UBPrzQ), and ask any questions on the dedicated channels for this chapter of material.
# 
# *Note - if you get error messages related to NumPy at any point, try restarting the kernel and re-running the imports (but <u>not</u> the cell with `pip install`s). This should fix the problem.*
...
if IN_COLAB:
    ...
    if not os.path.exists("chapter0_fundamentals"):
        get_ipython().system('wget https://github.com/callummcdougall/ARENA_2.0/archive/refs/heads/main.zip')
        get_ipython().system("unzip /content/main.zip 'ARENA_2.0-main/chapter0_fundamentals/exercises/*'")
...
# from transformer_lens.cautils.plotly_utils import *
...
import transformer_lens.rs.arthur.interview.arena_cnns_tests as tests
import torch
...
def as_strided_mv(mat: Float[Tensor, "i j"], vec: Float[Tensor, "j"]) -> Float[Tensor, "i"]:
    ...
    vec_stride = np_vec.strides[0]
    mat_shape_first = np_mat.shape[0]
    vec_shape = np_vec.shape[0]
    
    big_vec = np.lib.stride_tricks.as_strided(vec, shape=(mat_shape_first, vec_shape), strides=(0, vec_stride))

    assert big_vec.shape==mat.shape

    prod = big_vec * np_mat
    return torch.tensor(prod.sum(axis=1))
...
def conv1d_minimal_simple(x: Float[Tensor, "w"], weights: Float[Tensor, "kw"]) -> Float[Tensor, "ow"]:
    ...
    output_width = x.shape[0] - weights.shape[0] + 1

    xstrided = np.lib.stride_tricks.as_strided(
        x,
        shape=(output_width, weights.shape[0]),
